...
export const AI_GAME = "prompt-commander" as const;
...
const COPY = {
  zh: { label: "AI 探险护照", unlocked: "指令芯片已点亮", progress: (done: number, total: number) => `${done}/${total} 枚芯片`, hint: "你已经把模糊要求改造成了可执行指令。", account: "打开我的空间 →", saving: "正在保存指挥记录…" },
  en: { label: "AI ADVENTURE PASSPORT", unlocked: "PROMPT CHIP UNLOCKED", progress: (done: number, total: number) => `${done}/${total} CHIPS`, hint: "You turned a vague request into executable instructions.", account: "OPEN MY SPACE →", saving: "SAVING COMMAND RUN…" },
} as const;
...
export function AiQuestFooter({ lang, run }: { lang: "zh" | "en"; run: AiRunPayload }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      catch { setProgress({ done: 1, total: 5, best: { [AI_GAME]: { score: run.score, pct: run.pct, title: run.title } }, nextGame: "recommendation-tamer", daily: { date: "", game: AI_GAME, completed: false, streak: 0 }, isAnonymous: true }); }
    })();
  }, [run]);
