...
type Phase = "ready" | "playing" | "naming" | "finished";
type Lang = "zh" | "en";
type Round = {
  level: number;
  size: number;
...
const COPY = {
  zh: {
    ...
    namingTitle: "给自己起个称呼",
    namingHint: "这个称呼会印在你的战报上，方便朋友认出是谁发起的挑战。",
  ...
  en: {
    ...
    namingTitle: "NAME YOUR HUNTER",
    namingHint: "This name is printed on your report card so friends know who dared them.",
...
export default function ColorHunter() {
  ...
  function challengeUrl() {
    ...
    if (playerName) url.searchParams.set("from", playerName);
    return url.toString();
  }
  ...
  }, [phase, score, seed, currentPct, lang, playerName]);
  ...
  function submitName() {
    const name = sanitizeName(nameDraft).trim() || t.anon;
    setPlayerName(name);
    if (typeof window !== "undefined") window.localStorage.setItem("dx3xb_name", name);
    setCopied(false);
    setPhase("finished");
  }
  ...
  function reportText() {
    return t.reportText(playerName, score, resultTitle, currentPct, minDelta.toFixed(1), challengeUrl());
  }
  ...
  async function downloadReport() {
    ...
    try {
      ...
      link.download = `dx3xb-color-hunter-${playerName || "report"}.png`;
      link.href = dataUrl;
      link.click();
    } catch {
      /* capture failed */
  ...
  return (
    <main className="wrap">
      ...
      {phase === "naming" && (
        <section className="panel namingPanel">
          <p className="labKicker">{t.reportKicker}</p>
          <h2 className="pixel namingTitle">{t.namingTitle}</h2>
          ...
          <input
            ...
            onChange={(e) => setNameDraft(sanitizeName(e.target.value))}
            onKeyDown={(e) => {
              if (e.key === "Enter") submitName();
            }}
            autoFocus
          />
          <div className="namingMeta">
            <span>{score} · {resultTitle}</span>
            <span>{nameDraft.length}/{NAME_MAX}</span>
          </div>
          <div className="actions">
            <button className="btn coral" onClick={submitName}>
              {t.generate}
            </button>
          </div>
        </section>
      ...
      {phase === "finished" && (
        <section className="resultShell">
          <div className="reportCard" ref={reportRef}>
            <p className="labKicker">{t.reportKicker}</p>
            <p className="verdictLead">{t.verdictLead(playerName, currentPct)}</p>
            ...
            <div className="reportShare">
              ...
              <div className="qrText">
                ...
                <p className="qrUrl">color-hunter.dx3xb.com</p>
              </div>
            </div>
          ...
          <TrioFooter
            ...
            run={{
              ...
              lang,
              handle: playerName,
              stats: { minDelta: Number(minDelta.toFixed(1)), bestCombo, mistakes, level: Math.max(level - 1, 0) },
            }}
          />
